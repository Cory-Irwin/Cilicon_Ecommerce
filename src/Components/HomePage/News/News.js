...
Further tweaks are planned in the coming weeks, including social share links for wishlists and one-click upsell bundles. The dev team hinted at an AI-powered cart assistant in early testing.`,
    },
    {
        ...
        description: `In a surprise UX update aimed at night owls and low-light users, ShopDropper has launched an official Night Mode interface — a sleek, eye-comforting theme now live across all account pages and product listings.

But this isn’t just a cosmetic change: shoppers browsing after 9pm (in their local timezone) are automatically granted access to limited-time discounts and seasonal bundles designed specifically for off-peak hours. The design team promises more custom themes and accessibility tools in future updates.`,
    },
    {
        imgurl: News3,
        name: "Adrian",
...
export const News = () => {
    return (_jsx("section", { className: "bg-gray-200 py-20 px-4", children: _jsx("div", { className: "mx-auto max-w-screen-xl", children: _jsx("div", { className: "flex flex-col md:flex-row gap-10", children: newsData.map((item, index) => (_jsx(NewsComponent, { ...item }, index))) }) }) }));
};
